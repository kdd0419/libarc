...
for song in songlist:
    song_info.append({'id':song['id'],'name':song['title_localized']['en'], 'level':[diff['rating'] for diff in song['difficulties']]})
...
for i in range(len(song_info)*3):
    ...
    tmp_dic = {"song_name":song_info[i // 3]['name'], "difficulty": diff[i % 3], "level":song_info[i // 3]['level'][i % 3]}
    if len(score) <= 0:
